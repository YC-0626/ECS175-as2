...
class OBJLoader {
    ...
    parseVec3(vec_string) {
        let tokens = vec_string.trim().split(/\s+/).slice(1)
        let values = tokens.slice(0,3).map(x=>parseFloat(x))
        return values
    }
    ...
    {
        let tokens = face_string.split(/\s+/).slice(1)
        let indices = tokens.map(part => parseInt(part.split('/')[entry_index]) - 1)
        if (indices.length === 3){
            return indices
        }
        if (indices.length === 4) {
